...
<div class="col-md-12" >
  <div class="box box-warning ">
    <div class = "box-header with-border" id="Titulo_Center" >
      <h2 class = "box-title">Libreta de notas</h2>
...
/*
     var table = $('#tlb_export_excel').DataTable({
    dom: 'Bfrtip',
    buttons: [
      {
        extend: 'excel',
          ...
          exportOptions: {
              format: {
                body: function(data, row, column, node) {
                  ...
                  var colspan = $(node).attr('colspan');
                  var rowspan = $(node).attr('rowspan');

                  if (colspan) {
                    // Reemplazar el valor de la celda con una cadena vacía para las celdas adicionales
                    return Array(parseInt(colspan)).fill('').join('\t');
                  } else if (rowspan) {
                    // Reemplazar el valor de la celda con una cadena vacía para las celdas adicionales
                    return Array(parseInt(rowspan)).fill(data).join('\n');
                  } else {
                    return data;
                  }
                }
