...
function Show_Data_Docente($idDocente){
  $sql = "select id_docente, nombres, apellidos, dni, email, telefono, codigo, nivelId, tipo_docente from docentes where id_docente='$idDocente' ";
 $arreglo = array();
 if ($consulta = $this->conexion->conexion->query($sql)) {
  while ($consulta_VU = mysqli_fetch_assoc($consulta)) {
   $arreglo[]=$consulta_VU;
...
function Actualizar_Docente($nombreDocente,$apellidDocente,$dniDocente,$emailDocente,$telfDocente ,$codigDocente,$nivelDocente,$tipoDocente,$idDocente){
 $sql = "update docentes set nombres='$nombreDocente',apellidos='$apellidDocente',dni='$dniDocente',email='$emailDocente',telefono='$telfDocente',codigo='$codigDocente',tipo_docente='$tipoDocente', nivelId='$nivelDocente',updateDate= NOW() where id_docente='$idDocente' ";
            if ($consulta = $this->conexion->conexion->query($sql)) {
                return 1;
...
//REGISTRO DE DOCENTE CON ROL JARCODIN 2
 ...
 function Registrar_Docente($nombreDocente,$apellidDocente,$dniDocente,$emailDocente,$telfDocente ,$codigDocente,$nivelDocente,$tipoDocente){
  $sql = "INSERT INTO docentes(nombres,apellidos,dni,email,telefono,codigo,rolDocente,tipo_docente, nivelId,createDate)
  VALUES ('$nombreDocente','$apellidDocente','$dniDocente','$emailDocente','$telfDocente','$codigDocente','2','$tipoDocente','$nivelDocente',NOW())";
  if ($consulta = $this->conexion->conexion->query($sql)) {
   return 1;
 }else{
  return 0;
