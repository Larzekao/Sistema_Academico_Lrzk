    ...
    if ($verfivica==0) {
     ...
     if ($rolnombre=='DOCENTE') {
        $check =$MU->Chequear_Usuario_Ya_Generado($id_docente,$cbm_rol);
         ...
         if ($check==0) {
                ...
                $Upl=move_uploaded_file($_FILES["ImgPerfil"]["tmp_name"],"../../imagenes/usuarios/".$NameImg);
                 ...
                 $consulta = $MU->Registrar_Docente($id_docente,$nombre,$apellido,$usuario, $contra, $cbm_rol,$NameImg);
                echo $consulta;
               }else{echo 100;}      
        return;
      ...
      if ($check==0) {
         ...
         $consulta = $MU->Registrar_Alumno($id_alumno,$nombre,$apellido,$usuario, $contra, $cbm_rol,$NameImg);
         echo $consulta;
      }else{echo 100;}
         return;
    }
...
if (empty($_FILES["ImgPerfil"])) {
    	 ...
    	 $consulta = $MU->Registrar_Usuario($id_otro,$nombre,$apellido,$usuario,$contra, $cbm_rol,$NameImg);
    echo $consulta;
   // return;
   }
